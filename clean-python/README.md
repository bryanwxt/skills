...
| Spec | ✅ `docs/superpowers/specs/` | adds "Python implementation notes" | adds module cards |
| How the system splits into modules | — | — | ✅ |
| How each module is written in Python | — | ✅ protocols, dataclasses, typing, errors, DI | — |
| Plan | ✅ `writing-plans` | exact paths, tooling task, verification commands per task | interface-first ordering |
...
| Code review (commit range) | ✅ reviewer agent | Python checklist for the reviewer; checks feedback | design checklist |
...
**"Build a CLI that syncs invoices from our accounting API."** Brainstorming leads and asks one question at a time. The lens adds the Python choices that matter, such as sync or async (it recommends sync for a CLI unless there's heavy concurrent I/O) and whether to use a Protocol for the API client so tests can inject a fake. The spec gets a "Python implementation notes" section: package layout, error hierarchy, typing level, and required checks. `writing-plans` adds a tooling task if the repo has no linter or type checker, gives exact `src/…` and `tests/…` paths, and ends every task with `pytest`, `ruff`, and `mypy` commands.

**Implementing a plan task.** TDD runs red → green → refactor with pytest. Tests are parametrized for input tables, `pytest.raises` covers errors, and a fake client is injected rather than patched. The implementer follows the Python rules in the task brief: no mutable defaults or bare `except`, `raise … from e`, `with` for resources, and types and docstrings on public functions. It reports done only with the tool output attached, as `verification-before-completion` requires.
...
| `references/review-lens.md` | Pasted into the superpowers reviewer's requirements |
| `assets/spec-sections.md` | Python notes for a brainstorming spec |
...
| Prompt | Expected |
|---|---|
| "Build a small Python CLI that …" | Brainstorming leads; the spec has Python implementation notes; the plan has verification commands per task |
