...
| Implementation plan | ✅ `writing-plans` | shapes task order (interfaces first, abstractions not features) |
| Tests | ✅ test-first (`test-driven-development`) | says what to test: the module's public interface |
| Code review | ✅ `requesting-` / `receiving-code-review` | a design checklist for the reviewer; checks design feedback |
...
**Code review of a finished task.** The reviewer runs as a separate agent with a fixed template. The lens's checklist (`references/review-lens.md`) is added to the requirements field so the reviewer also looks at module and interface design. When feedback comes back, `receiving-code-review` checks it; the lens pushes back on "split it smaller" when that would create shallow pieces.
...
| `assets/spec-sections.md` | Sections to add to a brainstorming spec |
